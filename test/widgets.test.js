...
import { markdownDecorations } from '../src/markdown-decorations.js';

describe('Markdown Decoration Widgets', () => {
  it('Should define properly optimized replacer widgets', async () => {
    const fs = await import('fs');
    const path = await import('path');
    const decorationsSource = fs.readFileSync(path.resolve(__dirname, '../src/markdown-decorations.js'), 'utf-8');
    ...
    expect(decorationsSource).toMatch(/class MathInlineWidget extends WidgetType \{\s+constructor\(mathText\)/);
  });
});
