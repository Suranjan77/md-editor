...
- Note: recorded *after* the dependency had already shipped. That ordering
  ...
  `V3_IMPLEMENTATION_PLAN.md` §0.2; the decision itself stands.

## Context

`V3_IMPLEMENTATION_PLAN.md` Phase 1.2.5 / backlog item 7 deferred the decision on opening external URI links from PDFs in the system browser.
...
Add the `open` crate (version 5) as a dependency of `md3-shell`.
